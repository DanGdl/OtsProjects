...
crc calculate_crc(const uint8_t* const message, long int size) {
    crc remainder = ~0U;

    for (long int i = 0; i < size; ++i) {
        const uint32_t lookupIndex = (remainder ^ message[i]) & 0xFF;
        remainder = (remainder >> 8) ^ crc32_tab[lookupIndex];
    }
    remainder ^= ~0U;
    return remainder;
}
...
int main(int argc, char* argv[]) {
    ...
    if (stat(path, &stats) != 0) {
        ...
        crc remainder = calculate_crc(mapped, stats.st_size);
        printf("CRC32 remainder  for file %s: %X\n", path, remainder); // DEADBEEF
